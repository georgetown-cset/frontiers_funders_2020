...
gcp_cset_cnki.cset_cnki_journals_corpus where CAST(year as INT64) >= {year - 3} and  CAST(year as INT64) < {year} and
...
JournalID, ds_id from staging_merged_article_metadata.Papers_ds where year >= {year - 3} and year < {year})  a ON
...
wos_id  as id from staging_merged_article_metadata.PapersWithAbstracts_wos where CAST(Year as INT64) > {year - 3} and
 CAST(Year as INT64) < {year})  a ON j.wos_id = a.id
...
(select JournalID as id, CAST(PaperId as string) as PaperId, Year from gcp_cset_mag.PapersWithAbstracts)
 where Year  >= {year - 3} and Year  < {year}
...
select merged_id from gcp_cset_links_v2.article_merged_meta where year = {year}
