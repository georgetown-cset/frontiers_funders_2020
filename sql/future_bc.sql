/* Future data */
WITH cl as (
select cluster_id, article_id from september_20_clustering_experiments.future_bc_2_{for_year}_var{v}_latest
),
paper_cur as (
...
unclust as (
select id from (select id from paper_cur where year = {for_year} + 3) all_art left join cl  ON
